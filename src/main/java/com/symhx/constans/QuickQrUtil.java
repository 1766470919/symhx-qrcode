...
public class QuickQrUtil {
    ...
    public static ThreadLocal<AtomicInteger> TXT_INDEX_LOCAL;
    ...
    public static Font font(String name, int style, int fontSize) {
        ...
        return (Font)fontCache.computeIfAbsent(triple, (k) -> {
            return new Font((String)k.getLeft(), (Integer)k.getMiddle(), (Integer)k.getRight());
        });
    }

    private static int getIndex() {
        AtomicInteger integer = (AtomicInteger)TXT_INDEX_LOCAL.get();
        if (integer == null) {
            Class var1 = QuickQrUtil.class;
            synchronized(QuickQrUtil.class) {
                integer = (AtomicInteger)TXT_INDEX_LOCAL.get();
                if (integer == null) {
    ...
    static {
        ...
        RANDOM = new Random();
        TXT_INDEX_LOCAL = new ThreadLocal();
    }
}
